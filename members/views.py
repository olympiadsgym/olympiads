...
def forgot_password(request):
    """Allow a member to reset their password via registered email (UC-FP)."""
    sent = False
    error = None
    ...
    if request.method == 'POST':
        ...
        if not email:
            ...
            # Always show the success page regardless of whether the email exists
            ...
            if user is not None:
                ...
                plain_text = (
                    f"Hello,\n\n"
                    f"A password reset was requested for your OLYMPIADS portal account.\n\n"
                    f"Your new temporary password is: {temp_pw}\n\n"
                    f"Please log in at {PORTAL_LOGIN_URL} and change your password immediately.\n\n"
                    f"If you did not request this, please contact the gym staff.\n\n"
                    f"— OLYMPIADS Gym"
                )
...
<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>OLYMPIADS – Password Reset</title></head>
<body style="margin:0;padding:0;background:#0a0f1e;font-family:'Segoe UI',Helvetica,Arial,sans-serif;">
    ...
    <tr>
      <td align="center">
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;">
          ...
          <tr>
            <td align="center" style="padding-bottom:28px;">
              <p style="margin:0;font-size:22px;font-weight:800;letter-spacing:3px;color:#f9fafb;">OLYMPIADS</p>
              <p style="margin:4px 0 0;font-size:11px;color:#6b7280;text-transform:uppercase;letter-spacing:1.5px;">Gym Membership Portal</p>
            </td>
          </tr>
          ...
          <tr>
            <td style="background:#1a1d2e;border-radius:16px;overflow:hidden;border:1px solid rgba(255,255,255,0.07);">
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td style="padding:36px 36px 28px;">
                    <p style="margin:0 0 6px;font-size:13px;color:#9ca3af;text-transform:uppercase;letter-spacing:1px;">Password Reset</p>
                    <h1 style="margin:0 0 20px;font-size:24px;font-weight:700;color:#f9fafb;">Your new temporary password 🔑</h1>
                    <p style="margin:0 0 28px;font-size:15px;color:#d1d5db;line-height:1.6;">
                      A password reset was requested for your <strong style="color:#f5a623;">OLYMPIADS Gym</strong> portal account.
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0" style="background:#0f1117;border-radius:10px;border:1px solid rgba(245,166,35,0.2);margin-bottom:28px;">
                      ...
                      <tr>
                        <td style="border-top:1px solid rgba(255,255,255,0.05);padding:16px 24px;">
                          <p style="margin:0 0 4px;font-size:11px;color:#6b7280;text-transform:uppercase;letter-spacing:1px;">New Temporary Password</p>
                          <p style="margin:0;font-size:16px;font-weight:700;color:#f5a623;letter-spacing:2px;font-family:monospace;">{temp_pw}</p>
                        </td>
                      </tr>
                    </table>
                    ...
                    <p style="margin:0;font-size:13px;color:#6b7280;line-height:1.6;">
                      If you did not request a password reset, please contact gym staff immediately.
                    </p>
                  </td>
                </tr>
              </table>
          ...
          <tr>
            <td align="center" style="padding:24px 0 0;">
              <p style="margin:0;font-size:12px;color:#4b5563;">© OLYMPIADS Gym · This is an automated message, please do not reply.</p>
                ...
                try:
                    msg = EmailMultiAlternatives(
                        subject="OLYMPIADS Gym — Your New Temporary Password",
                        body=plain_text,
                        from_email=settings.DEFAULT_FROM_EMAIL,
                        to=[email],
                    )
                    ...
                    # Fall through — still show "sent" to prevent enumeration
